...
class Building3DService {
  ...
  async loadModels() {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.log(
          `🏢 Loaded ${this.models.length} building models from backend`
        );
        return this.models;
      }
    } catch (error) {
  ...
  generateBuildingPositions(parcelCoords, count) {
    ...
    while (positions.length < count && attempts < maxAttempts) {
      ...
      const lng =
        minLng + margin + Math.random() * (maxLng - minLng - 2 * margin);
      const lat =
        minLat + margin + Math.random() * (maxLat - minLat - 2 * margin);
    ...
    map.addLayer({
      ...
      paint: {
        "fill-extrusion-color": [
          ...
          ["get", "height"],
          0,
          "#e2e8f0", // Light blue-gray for short buildings
          10,
          "#94a3b8", // Medium blue-gray
          20,
          "#64748b", // Darker blue-gray for tall buildings
        ],
