...
class GeoapifyService {
  constructor() {
    if (!API_KEY) {
      console.warn(
        "Geoapify API Key is missing! Please add REACT_APP_GEOAPIFY_KEY to your .env file."
      );
    }
  }
  ...
  async getCityDistricts(cityId, cityName) {
    ...
    try {
      console.log(
        `🌍 Fetching districts for ${cityName} using Hardcoded List...`
      );

      const districtNames =
        CITY_DISTRICTS[cityId.toLowerCase()] || CITY_DISTRICTS[cityId];

      if (districtNames) {
        return await this.fetchDistrictsByNames(
          districtNames,
          cityName,
          cityId
        );
  ...
  async fetchDistrictsByNames(districtNames, cityName, cityId) {
    const promises = districtNames.map(async (name, index) => {
      try {
        ...
        const query = name.toLowerCase().includes(cityName.toLowerCase())
          ? name
          : `${name}, ${cityName}`;
  ...
  async getAmenities(
    ...
    try {
      ...
      return data.features.map((f) => ({
        name:
          f.properties.name || f.properties.address_line1 || "Unknown Place",
