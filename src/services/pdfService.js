...
class PDFService {
  generateReport(report) {
    ...
    const pricingData = [
      ...
      [
        "Market Trend",
        `${pricing.marketTrend.trend} (${pricing.marketTrend.growthRate})`,
      ],
    ];
    ...
    doc.setTextColor(0, 0, 0);
    doc.text(
      `Overall Score: ${buildability.score}/100 - Grade ${buildability.grade}`,
      margin,
      yPos
    );
    yPos += 10;
    ...
    if (traffic) {
      ...
      const trafficData = [
        [
          ...
          `${traffic.dailyTrips.toLocaleString()} trips/day`,
        ],
        [
          "Peak Hour Trips",
          `${traffic.peakHourTrips.toLocaleString()} trips/hour`,
        ],
        ["Based On", `${traffic.unitCount} ${traffic.unitType}`],
        [
          "Congestion Level",
          `${traffic.congestion.level} - ${traffic.congestion.description}`,
        ],
      ];
      ...
      const noteText =
        "Note: Traffic estimates based on ITE Trip Generation rates. Actual traffic may vary.";
    ...
    if (aqiForecast && aqiForecast.length > 0) {
      ...
      const avgAQI = (
        aqiForecast.reduce((a, b) => a + b, 0) / aqiForecast.length
      ).toFixed(1);
    ...
    scenarios.forEach((scenario, idx) => {
      ...
      const scenarioData = [
        ...
        [
          "Number of Floors",
          scenario.floors ? scenario.floors.toString() : "N/A",
        ],
        [
          "Built-up Area",
          scenario.builtArea
    ...
    if (recommendations && recommendations.length > 0) {
      recommendations.forEach((rec, idx) => {
        ...
        if (rec.description) {
          doc.setFont(undefined, "normal");
          doc.setTextColor(0, 0, 0);
          doc.text(rec.description, margin + 5, yPos, {
    ...
    // Save the PDF
    doc.save(
      `Property_Analysis_Report_${new Date().toISOString().split("T")[0]}.pdf`
    );
  }
  ...
  addCoverPage(doc, pageWidth, pageHeight, primaryColor) {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(
      "This report contains detailed analysis of the property including",
      pageWidth / 2,
      pageHeight / 2 + 45,
      { align: "center" }
    );
    doc.text(
      "zoning regulations, pricing, amenities, and development scenarios",
      pageWidth / 2,
      pageHeight / 2 + 52,
      { align: "center" }
    );
    ...
    doc.text("Urban Development Analyzer", pageWidth / 2, pageHeight - 30, {
      align: "center",
    });
  }
  ...
  addFooter(doc, pageHeight, margin) {
    ...
    doc.setFont(undefined, "italic");
    const disclaimer =
      "Disclaimer: This report is generated based on available data and ML predictions. " +
