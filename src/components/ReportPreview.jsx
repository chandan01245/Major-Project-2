...
const ReportPreview = ({ report, onClose, onDownload }) => {
  ...
  // Helpers for flood visuals
  const inchesToMeters = (inches) => (inches || 0) * 0.0254;

  const severityColor = (level) => {
    if (!level) return "bg-emerald-100 text-emerald-600";
  ...
  const renderFutureBars = (future = []) => {
    if (!future || future.length === 0) return null;
    const depths = future.map((f) => f.depthInches || 0);
    const maxDepth = Math.max(...depths, 1);

    return (
      <div className="flex items-end gap-2">
        {future.map((f, i) => {
          const h = Math.max(4, ((f.depthInches || 0) / maxDepth) * 48); // px height
          const color =
  ...
  const BuildingWater = ({ depthInches = 0 }) => {
    const depthM = inchesToMeters(depthInches);
    const buildingHeightM = 12; // illustrative building height
    const fillPct = Math.min(1, depthM / buildingHeightM);
    const buildingH = 80; // px
    const waterH = Math.round(buildingH * fillPct);

    return (
      <svg width="48" height="88" viewBox="0 0 48 88" className="ml-3">
        <rect
          x="8"
          y="8"
          width="32"
        ...
        <rect
          x="8"
          y={8 + (buildingH - waterH)}
          width="32"
          height={waterH}
          fill="#60a5fa"
          opacity="0.9"
        />
        <text x="24" y="86" fontSize="9" textAnchor="middle" fill="#475569">
          {depthM.toFixed(2)}m
        </text>
      </svg>
    );
          ...
          <div className="flex items-center gap-4 text-xs text-emerald-100">
            ...
            <div className="flex items-center gap-2 bg-white/20 px-2 py-1 rounded">
              ...
              <span className="font-semibold">
                Confidence: {(mlConfidence * 100).toFixed(0)}%
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-5 border-2 border-blue-200 hover:border-blue-400 transition-all shadow-sm hover:shadow-md group">
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-blue-700 bg-blue-100 px-2 py-1 rounded-full w-fit">
                    ...
                    <span className="font-semibold">
                      {pricing.marketTrend?.trend || "Rising"} Market
                    </span>
                  </div>
                </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {scenarios.map((scenario, idx) => (
                  <div
                    key={idx}
                    className="relative overflow-hidden bg-gradient-to-br from-white to-slate-50 border-2 border-slate-200 hover:border-emerald-300 rounded-xl p-5 shadow-sm hover:shadow-lg transition-all group"
                  >
                      ...
                      {scenario.builtArea && (
                        <div className="flex justify-between items-center p-2 bg-emerald-50 rounded-lg border border-emerald-100">
                          ...
                          <span className="text-sm font-bold text-slate-800">
                            {scenario.builtArea.toLocaleString()}{" "}
              ...
              <section>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-emerald-50 p-3 rounded-xl border border-emerald-200">
                    ...
                    <p className="text-xl font-bold text-slate-800">
                      {traffic.dailyTrips.toLocaleString()}
                    </p>
                    <p className="text-xs text-slate-500 mt-1">
                      {traffic.unitCount} {traffic.unitType}
                    </p>
                  </div>
                  <div className="bg-emerald-50 p-3 rounded-xl border border-emerald-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        >
                          ...
                          <div className="flex gap-2 text-xs text-slate-500">
                            <span className="flex items-center gap-1">
                              <Car className="w-3 h-3" /> {s.drivingTime || 0}{" "}
                              min
                            </span>
                ...
                {((amenities.metro && amenities.metro.length > 0) ||
                  ...
                  <div className="bg-white border border-emerald-200 rounded-xl p-4">
                    ...
                    <ul className="space-y-2">
                      {(amenities.metro || amenities.transport || []).map(
                        (s, i) => (
                          <li
                            key={i}
                            className="border-b border-slate-100 pb-2 last:border-0 last:pb-0"
              ...
              <div className="space-y-3">
                ...
                {floodRisk && (
                  <div className="bg-white rounded-xl border-2 border-blue-200 overflow-hidden">
                    <div className="flex items-center gap-2 p-3 bg-gradient-to-r from-blue-50 to-cyan-50 border-b border-blue-200">
                      <div className="bg-blue-100 p-2 rounded-lg">
                    ...
                    <div className="p-4 border-b border-slate-200">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`px-3 py-1 rounded-full text-xs font-semibold ${severityColor(
                              floodRisk.current?.riskLevel
                            )}`}
                          >
                            {floodRisk.current?.riskLevel || "Low"}
                          </div>
                        ...
                        <div className="flex items-center gap-3">
                          {renderFutureBars(floodRisk.future)}
                          <BuildingWater
                            depthInches={floodRisk.current?.depthInches}
                          />
                        </div>
                      </div>
                      <div className="text-center">
                        <div
                          className={`w-20 h-20 rounded-xl flex items-center justify-center mx-auto mb-3 ${
                            floodRisk.current?.riskLevel === "Critical"
                              ? "bg-red-100 text-red-600"
                              : floodRisk.current?.riskLevel === "High"
                              ? "bg-orange-100 text-orange-600"
                              : floodRisk.current?.riskLevel === "Moderate"
                              ? "bg-amber-100 text-amber-600"
                              : "bg-emerald-100 text-emerald-600"
                          }`}
                        >
                        ...
                        <p className="text-sm font-semibold text-slate-600 mb-2">
                          Current Risk Score: {floodRisk.current?.riskScore}
                        </p>
                        <p className="text-xs text-slate-500">
                          {floodRisk.current?.description}
                        </p>
                      </div>
                      ...
                      <div className="mt-4 grid grid-cols-2 gap-3">
                        ...
                        <div className="bg-cyan-50 p-3 rounded-lg text-center">
                          ...
                          <p className="text-lg font-bold text-cyan-600">
                            {floodRisk.current?.elevation || 0}m
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Future Projections */}
                    {floodRisk.future && floodRisk.future.length > 0 && (
                      <div className="p-4">
                        <h6 className="text-xs font-bold text-slate-700 mb-3 uppercase tracking-wide">
                          Future Climate Projections
                        </h6>
                        <div className="space-y-2">
                          {floodRisk.future.map((projection, idx) => (
                            <div
                              key={idx}
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                >
                                  {projection.year.replace(" Years", "y")}
                                </div>
                                <div>
            ...
            <div className="pt-4 border-t border-emerald-200">
              <p className="text-xs text-slate-500 text-center">
                Report ID: {parcelInfo.area.toString().slice(-6)} • Generated:{" "}
